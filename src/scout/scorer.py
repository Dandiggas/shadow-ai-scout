...
def _matches(requirement: Requirement, claim: EvidenceClaim) -> bool:
    haystack = f"{claim.risk_category} {claim.claim_text} {claim.evidence_quote}".lower()
    return any(keyword.lower() in haystack for keyword in requirement.keywords)
...
def assess_requirement(requirement: Requirement, claims: list[EvidenceClaim]) -> RequirementAssessment:
    ...
    if matching:
        best = max(matching, key=lambda claim: claim.confidence)
...
def score_tool(tool_name: str, requirements: list[Requirement], claims: list[EvidenceClaim]) -> ToolVerdict:
    ...
    for claim in relevant_claims:
        ...
        score += round((category_weight + severity_weight) * confidence_factor)
    ...
    failed_policy = [a.label for a in assessments if a.status != "pass"]
    top_risks = sorted(relevant_claims, key=lambda c: (c.severity, c.confidence), reverse=True)[:2]
